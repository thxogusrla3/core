...
- 싱글톤, 프로토타입, 웹 스코프 3가지가 있음.

## 1. Singleton 스코프
- 기본 스코프, 스프링 컨테이너의 시작과 함께 생성되어 스프링 컨테이너가 종료될 때 까지 유지된다.
- 싱글톤 빈을 여러번 조회 시 같은 객체를 반환하며, 소멸할 때까지 관리해준다.

## 2. Prototype 스코프
- 프로토타입 빈의 생서오가 의존관계 주입 및 초기화 까지만 관여하고 소멸은 관리하지 않음.
...
- 이 문제를 해결하기 위해 Provider 가 사용되었다.

# 프로토타입 스코프 - 싱글톤 빈과 함께 사용시 Provider 로 문제 해결
> Dependency Lookup(DL) 이란 필요한 시점에 직접 Spring Bean에 접근하기 위해 컨테이너가 제공하는 API를 이용하여 의존성을 찾는 방법이다.
...
### 1. 웹 스코프 종류
- request: HTTP Request (요청) 하나가 들어오고 나갈 때 까지 유지되는 스코프, 각각의 HTTP 요청마다 별도의 빈 인스턴스가 생성되고 관리된다.
